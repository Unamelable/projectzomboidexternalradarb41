...
BUILDING RENDERER
  ...
  • Color-coded by status: cleared (green), visited/partial (red),
    untouched (dark blue), building-touched (dim outline)
...
BUILDING CLEARING SYSTEM
  ...
  • Persists cleared buildings to
    zombiemap_cleaningbuildings_{savename}.txt
...
CHUNK CLEARING SYSTEM
  ...
  • Persists cleared chunks to
    zombiemap_cleaningall_{savename}.txt
...
BACKGROUND MUSIC (CHIPTUNE)
  • Plays tracker module files (.XM/.IT/.MOD) encoded as base64 (.B64)
